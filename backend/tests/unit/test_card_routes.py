...
class TestCardRoutes:
    ...
    def test_submit_card_unauthenticated(self, client):
        ...
        response = client.post("/api/submissions", json={"name": "New Business"})
    ...
    def test_get_user_submissions(self, client, user_token, db_session, regular_user):
        ...
        submission = CardSubmission(name="My Submission", submitted_by=regular_user.id)
        db_session.session.add(submission)
        db_session.session.commit()

        response = client.get("/api/submissions", headers={"Authorization": f"Bearer {user_token}"})
    ...
    def test_suggest_card_edit(self, client, user_token, sample_card):
        ...
        response = client.post(
            f"/api/cards/{sample_card.id}/suggest-edit",
            headers={"Authorization": f"Bearer {user_token}"},
