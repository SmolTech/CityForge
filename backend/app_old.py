...
class CardSubmission(db.Model):
    ...
    card_id = db.Column(db.Integer, db.ForeignKey('cards.id'), nullable=True)  # If approved and converted
    created_date = db.Column(db.DateTime, default=datetime.utcnow)
...
class QuickAccessItem(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    identifier = db.Column(db.String(50), nullable=False, unique=True, index=True)  # e.g. 'emergency', 'city-hall'
    title = db.Column(db.String(100), nullable=False)
    subtitle = db.Column(db.String(100), nullable=False)
    phone = db.Column(db.String(20), nullable=False)
    color = db.Column(db.String(20), nullable=False, default='blue')  # blue, green, purple, red, etc.
...
class ResourceItem(db.Model):
    ...
    category = db.Column(db.String(100), nullable=False, index=True)  # Keep string for backward compatibility
    category_id = db.Column(db.Integer, db.ForeignKey('resource_categories.id'), nullable=True)  # Future use
...
def get_business(business_id, slug=None):
    ...
    if slug and slug != card.slug:
        return jsonify({
            'redirect': f'/business/{business_id}/{card.slug}'
        }), 301
...
def get_tags():
    ...
    tags_with_counts = db.session.query(
        ...
        card_tags, Tag.id == card_tags.c.tag_id, isouter=True
    ).group_by(Tag.id, Tag.name).order_by(Tag.name.asc()).all()
...
def get_resources_config():
    ...
    try:
        ...
        config['site'] = {
            ...
            'description': config_dict.get('site_description', 'Helping connect people to the resources available to them.'),
            'domain': config_dict.get('site_domain', 'community.local')
        }

        # Resources configuration
        config['title'] = config_dict.get('resources_title', 'Local Resources')
        config['description'] = config_dict.get('resources_description', 'Essential links to local services and information')
        ...
        if not config.get('footer'):
            config['footer'] = {
                ...
                'description': config_dict.get('footer_description', 'If you know of an important local resource that should be included on this page, please let us know.'),
...
def get_quick_access():
    ...
    try:
        items = QuickAccessItem.query.filter_by(is_active=True).order_by(QuickAccessItem.display_order, QuickAccessItem.id).all()
...
def get_resource_items():
    ...
    try:
        ...
        items = query.order_by(ResourceItem.category, ResourceItem.display_order, ResourceItem.title).all()
        return jsonify([item.to_dict() for item in items])
    except Exception as e:
...
def get_resource_categories():
    ...
    try:
        ...
        result = db.session.query(ResourceItem.category).filter(ResourceItem.is_active == True).distinct().all()
    ...
    try:
        ...
        return jsonify({
            'site': {
                ...
                'directoryDescription': config_dict.get('site_directory_description', 'Discover local resources and community information.'),
    ...
    try:
        ...
        for hit in response['hits']['hits']:
            ...
            result = {
                ...
                'url': source['url'],
                'page_url': source.get('page_url', source['url']),  # Use page_url if available, fallback to main url
